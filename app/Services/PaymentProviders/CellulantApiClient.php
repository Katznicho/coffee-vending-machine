...
class CellulantApiClient
{
    ...
    public function accessToken(): string
    {
        ...
        $cacheKey = sprintf(
            'cellulant_oauth_%s_%s',
            $this->settings->id,
            $this->settings->updated_at?->getTimestamp() ?? 0,
        );

        return Cache::remember($cacheKey, 3500, function () use ($username, $password) {
    ...
    protected function authorizedRequest(string $method, string $url, array $data = []): Response
    {
